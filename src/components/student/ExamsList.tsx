...
import { fetchEnrolledExamsPage, fetchEnrolledExamTermKeys, getStudentExamStatus } from '../../lib/studentExams';
...
export function ExamsList() {
    ...
    // Load term keys once on mount
    useEffect(() => {
        if (!user) return;
        fetchEnrolledExamTermKeys(user.id).then(keys => setAvailableTerms(keys));
    }, [user]);
    ...
    useEffect(() => {
        ...
        fetchEnrolledExamsPage(user.id, {
    ...
    }, [user, debouncedSearch, termFilter, currentPage]);

    // Status filter is client-side (requires computed logic from exam_attempts)
    const filteredExams = statusFilter === 'all'
        ? exams
        : exams.filter(e => getStudentExamStatus(e) === statusFilter);
